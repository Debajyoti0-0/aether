...
- **Environment incident (documented honestly):** OneDrive sync repeatedly deleted working-tree files under `internal/planner/` mid-sprint, producing one bad commit (`dda0477`) that recorded deletions; restored from git history across fix commits `442b67e/0e56ea3/a65076e/e174078/2faa9b3` (atomic write+commit pattern). Recommendation recorded: run the repo outside OneDrive-synced paths or pause sync during sprints.
