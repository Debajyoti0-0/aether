...
| 16 | S42-1 revoke MkdirAll | `internal/cli/serve_cert.go` | revoke ensures the PKI directory exists (`os.MkdirAll(tsCertDir, 0o700)`) before appending to `revoked.txt` | `serve cert revoke --dir %TEMP%\ae41-s42-1 --operator bob` → exit 0 |
